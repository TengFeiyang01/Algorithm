...
void solve() {
    ...
    std::vector dp(n, std::vector<Z>(4));
    // dp是不包含der的
    dp[0][0] = 23;
    dp[0][1] = 1, dp[0][2] = 1, dp[0][3] = 1;
    dp[1][1] = 26, dp[1][2] = 26, dp[1][3] = 26;
    dp[1][0] = 26 * 23;
    // f是即不包含der 也不包含red
    auto f = dp;
    for (int i = 2; i < n; ++i) {
        Z tmp = dp[i - 1][0] + dp[i - 1][1] + dp[i - 1][2] + dp[i - 1][3];
        dp[i][0] = 23 * tmp;
        dp[i][1] = tmp;
        dp[i][2] = tmp;
        dp[i][3] = tmp - dp[i - 2][1];
    ...
    for (int i = 2; i < n; ++i) {
    	Z tmp = f[i - 1][0] + f[i - 1][1] + f[i - 1][2] + f[i - 1][3];
    	f[i][0] = 23 * tmp;
    	f[i][1] = tmp - f[i - 2][3];
    	f[i][2] = tmp;
    	f[i][3] = tmp - f[i - 2][1];
