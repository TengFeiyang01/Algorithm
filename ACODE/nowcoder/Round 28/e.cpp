...
void solve() {
	int n, k;
	std::cin >> n >> k;
	Z odd = (k + 1) / 2, even = k - odd;
	Z oddPos = (n + 1) / 2, evenPos = n - oddPos;

    std::cout << power(Z(even), n) + 1LL * power(Z(even), n / 3) * power(Z(odd), n - n / 3) + 
        power(Z(even), (1 + (n - 2) / 3)) * 1LL * power(Z(odd), n - (1 + (n - 2) / 3)) + 
        power(Z(even), (1 + (n - 1) / 3)) * 1LL * power(Z(odd), n - (1 + (n - 1) / 3));
}
