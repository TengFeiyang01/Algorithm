...
void solve() {
    ...
    Z ans = 1;
    //长度为2
    ans *= power(Z(3), k - 1);  //2 2
    ans *= power(Z(4), n - k);  //2 3
    //长度为3
    ans *= power(Z(4), (k - 1) * (k - 2) / 2); //2 2 2 | 3 3 3 
    ans *= power(Z(6), (n - k - 1) * (n - k) / 2);  //2 2 3
    ans *= power(Z(6),  (k - 1) * (n - k));  //3 2 3
