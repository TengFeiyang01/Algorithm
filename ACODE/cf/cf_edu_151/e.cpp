...
//0-1数组a 至少一个0和一个 每次交换相邻的（0，1），问你最终的个数
...
void solve() {
    ...
    std::vector dp(n + 1, std::vector<Z>(k + 1));
	dp[0][0] = 1;
	for (int i = 0; i < n; ++i) {
		int r = std::min(std::min(i + 1, S), s[i] + 60);
		int l = std::max(1, s[i] - 60);
		for (int j = r; j >= l; --j) {
			int need = abs(pos[j - 1] - i);
			for (int t = k; t >= need; --t) {
				dp[j][t] += dp[j - 1][t - need];
			}
		}
	}
	Z ans = 0;
	for (int i = k; i >= 0; i -= 2) ans += dp[S][i];
	std::cout << ans << "\n";
}
