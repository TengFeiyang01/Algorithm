...
void solve() {
    int n;
    std::cin >> n;
    std::vector<int> a(n), ls(n, -1), rs(n, -1), stk(n);
    for (int i = 0; i < n; ++i) {
    	std::cin >> a[i];
    }
    int top = -1;
    for (int i = 0; i < n; ++i) {
    	int pos = top;
    	while (pos >= 0 and a[stk[pos]] > a[i]) --pos;
    	if (pos >= 0) rs[stk[pos]] = i;
    	if (pos < top) ls[i] = stk[pos + 1];
    	stk[top = ++pos] = i;
    }

    std::function<int(int, int, int)> dfs = [&](int u, int l, int r) {
    	Z c1 = 1, c2 = 1;
    	if (ls[u] != -1) c1 = dfs(ls[u], l, u - 1);
    	if (rs[u] != -1) c2 = dfs(rs[u], u + 1, r);
    	Z ans = c1 * c2;
    	if (l == 0 and r == n - 1) {
