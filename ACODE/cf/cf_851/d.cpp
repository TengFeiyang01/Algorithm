...
//数轴最终留下的点必然是由子集序列中相邻的两个点相遇所形成的
//因此可以枚举点对(xi,xj)，统计每个点对能够贡献的答案是多少,如果想让xi
//和xj相遇成为最后留下的点，那么首先在xi<xk<xj范围内的点不能选,我们还要保证xi向左移动xj向右移动
//记d=xj−xi，那么xi−d≤xk<xi和xj<xk≤xj+d−1内的点不能选
...
void solve() {
	...
	for (int i = 0; i < n - 1; ++i) {
		for (int j = i + 1; j < n; ++j) {
			...
			//找小于a[i]-d的数的个数
			int r = (n - (std::lower_bound(a.begin(), a.end(), a[j] + d) - a.begin()) + 1);
			ans += power(Z(2), l + r);
		}
	}
	std::cout << ans << "\n";
