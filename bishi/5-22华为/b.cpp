...
void solve() {
    ...
    std::function<bool(int, int)> check = [&](int i, int j) -> bool {
    	return digit[i][j] or !(s[i][j] == "B");
    };
    std::function<void(std::vector<std::vector<int>>&, std::priority_queue<std::pair<int, int>>&)> update = [&](std::vector<std::vector<int>> &dp, std::priority_queue<std::pair<int, int> > &pq) ->void {
    	while (!pq.empty()) {
    		auto t = pq.top();
    		pq.pop();
