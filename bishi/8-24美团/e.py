...
pos = [defaultdict(SortedList), defaultdict(SortedList)]
...
for i in range(q):
    ...
    qry[t].append([l, r, k, i])

for i in range(2):
    segment_tree = SegmentTree(max)
    for j in range(1, n + 1):
        if i:
            segment_tree.update_point(i, c[i])
        else:
            segment_tree.update_point(i, a[i])
    print(a[:i+1])
    print(a[:i+1])
    ...
    for qr in qry[i]:
        l, r, k, idx = qr
        for j in range(k):
            p = segment_tree.query_interval(l, r)
            if p == 0:
                ans[idx].append(-1)
                break
            it = pos[i][p].bisect_left(l)
            ans[idx].append(it)
            segment_tree.update_point(it, -p)
            pos[i][p].remove(it)
