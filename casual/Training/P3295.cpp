...
void solve() {
	int n, m;
	std::cin >> n >> m;
	std::vector fa(n + 1, std::vector<int>(19));
	...
	std::function<int(int, int)> find = [&](int x, int k) ->int {
		return fa[x][k] == x ? x : fa[x][k] = find(fa[x][k], k);
	};

	std::function<void(int, int, int)> merge = [&](int x, int y, int k) ->void {
		x = find(x, k);
		y = find(y, k);
		if (x != y) fa[x][k] = y;
	};
	...
	for (int i = 1, l1, l2, r2, r1; i <= m; ++i) {
		std::cin >> l1 >> r1 >> l2 >> r2;
		for (int k = Log[r1 - l1 + 1]; k >= 0; --k) {
			if (l1 + (1 << k) - 1 <= r1) {
				merge(l1, l2, k);
				l1 += 1 << k;
				l2 += 1 << k;
