...
void solve() {
    ...
    //n-1中选k个 答案是C(n-1, k) * m * (m - 1) ^ k 
    //m是第一块的颜色 m-1的其余砖块的颜色
    // std::cout << C(n - 1, k) * m * power(Z(m - 1), k) << "\n";

    // std::vector<std::vector<Z>> dp(n, std::vector<Z>(k + 1));
    // dp[0][0] = m;
    // for (int i = 1; i < n; ++i)
    //     for (int j = 0; j <= k; ++j)
    //         dp[i][j] = dp[i - 1][j] + (j > 0 ? dp[i - 1][j - 1] * (m - 1) : Z(0));
    // std::cout << dp[n - 1][k];
