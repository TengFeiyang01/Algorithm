...
void solve() {
    ...
    std::vector<std::array<Z, 2>> dp1(n), dp2(n);
    Z ans = 0;
    ...
    auto dfs2 = [&](auto &&self, int u, int fa) -> void {
    	...
    	for (auto v : adj[u]) {
    		if (v == fa) continue;
    		dp1[v][0] = dp2[u][1] - dp2[v][0] + dp1[u][1];
    		dp1[v][1] = dp2[u][0] - dp2[v][1] + dp1[u][0];
    		ans += a[v] * (dp1[v][0] - dp1[v][1]) * (dp2[v][0] + dp2[v][1]);
    		ans += a[u] * (dp1[v][0] + dp1[v][1]) * (dp2[v][1] - dp2[v][0]);
    		self(self, v, u);
    	}
    };
    dfs2(dfs2, 0, -1);
